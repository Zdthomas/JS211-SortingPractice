  ...
  const result = stringsToNumbs(strNums);
  ...
  const sumEvens = (evenNum) => { 
    ...
    const evenNum = strNums

    const evenValue = sumEvens(evenNum);
...
// );
  ...
  const weatherStates = weather.reduce(
    ...
    (result, curr) => {
        ...
        console.log('curr', curr.weather_state_name);

        if (!result.includes(curr.weather_state_name)) {
        
        result.push(curr.weather_state_name) 
    }
...
//   Couldn't get it to work like you did for some reason. I had to change the code around for it work on repl.it. 
  ...
  const idealTemp = weather.find(temp => temp.min_temp === 15.915);
...
//         // console.log('result', result)
//         // console.log('curr', curr.min_temp)

//         if(result.includes(curr.min_temp === 15.915)) {
